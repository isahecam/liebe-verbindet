...
**Tech stack:** Astro 4 + React 18 (islands) + Tailwind CSS 3 + TypeScript
...
**Custom Tailwind config** (`tailwind.config.mjs`):
- Extra breakpoints: `xs` (425px), `xsm` (520px) — used heavily for responsive layout.
- Custom color palette: `valentines-day` (50–950 pink shades).
- Custom animation: `animate-text-gradient` for gradient text cycling.
- Plugin: `@midudev/tailwind-animations` (provides `animate-float`, etc.).
